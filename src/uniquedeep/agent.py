...
IMPORTANT: When you need to think before acting or responding (e.g., analyzing a request, deciding which tool to use, or planning steps), you MUST wrap your thoughts in <thinking>...</thinking> tags. This helps separate your internal reasoning from your final response."""
    ...
    def _process_text_chunk_with_tags(self, text: str, emitter: StreamEventEmitter):
        """
        处理可能包含 <thinking> 标签的文本流
        """
        if not text:
            return

        # 状态机处理流式标签
        # 简单实现：扫描文本，根据状态输出 thinking 或 text
        
        current_pos = 0
        while current_pos < len(text):
            if not self._in_thinking_tag:
                # 寻找 <thinking>
                start_tag_idx = text.find("<thinking>", current_pos)
                if start_tag_idx != -1:
                    # 输出标签前的内容为 text
                    pre_text = text[current_pos:start_tag_idx]
                    if pre_text:
                        yield emitter.text(pre_text)
                    
                    self._in_thinking_tag = True
                    current_pos = start_tag_idx + len("<thinking>")
                else:
                    # 检查是否有部分标签在末尾（例如 "<th"）
                    # 这是一个简化的处理，暂不处理跨 chunk 的部分标签匹配，
                    # 因为 <thinking> 较长，分片概率低，且即使漏掉前几个字符也不影响大局。
                    # 如果需要完美处理，需要更复杂的 buffer。
                    # 这里直接输出剩余内容
                    yield emitter.text(text[current_pos:])
                    current_pos = len(text)
            else:
                # 寻找 </thinking>
                end_tag_idx = text.find("</thinking>", current_pos)
                if end_tag_idx != -1:
                    # 输出标签前的内容为 thinking
                    thinking_content = text[current_pos:end_tag_idx]
                    if thinking_content:
                        yield emitter.thinking(thinking_content)
                    
                    self._in_thinking_tag = False
                    current_pos = end_tag_idx + len("</thinking>")
                else:
                    # 整个都是 thinking
                    yield emitter.thinking(text[current_pos:])
                    current_pos = len(text)
    ...
    def _process_chunk_content(
        ...
        # Anthropic 的 thinking 处理逻辑
        is_anthropic = (self.provider == "anthropic" or "claude" in self.model_name.lower())
        
        if isinstance(content, str):
            if content:
                # 如果是 Anthropic 且启用了 thinking，尝试解析 XML 标签
                if is_anthropic and self.enable_thinking:
        ...
        # 检查这个 chunk 是否包含 thinking 块
        # 如果包含 thinking 块，说明 Anthropic 正在使用 Extended Thinking (Native)
        ...
        for raw_block in blocks:
            ...
            if block_type in ("thinking", "reasoning"):
                ...
                if text:
                    # 如果使用了 Native Extended Thinking，通常不需要再解析 tags，
                    # 但 Claude 有时会混合使用。
                    # 如果启用了 thinking，我们优先尝试解析 tags
                    if is_anthropic and self.enable_thinking:
                         # 即使有 tool_use，如果内容被包裹在 tags 里，也会被正确处理
                         # 如果没有 tags，我们还是保留之前的 heuristic 作为兜底吗？
                         # 既然加了 Prompt 指令，我们期望它使用 tags。
                         # 如果它没用 tags 但用了 tool，之前的 heuristic 依然有价值。
                         
                         # 这里稍微复杂一点：我们先尝试用 tags 解析
                         # 如果解析过程中发现它确实用了 tags（即状态机发生了切换），那最好。
                         # 如果没有 tags，text 就会被原样输出为 text 事件。
                         # 这时我们可以在 _process_text_chunk_with_tags 里集成之前的 heuristic。
                         
                         # 简化策略：
                         # 1. 先通过 tags 解析器
                         # 2. 如果解析器输出的是 text 事件，我们再应用 heuristic 判断是否要转 thinking
                         
                         # 但 generator 比较难拦截。
                         # 我们直接在 _process_text_chunk_with_tags 中实现逻辑：
                         # 如果 text 不含 tags，则应用 heuristic。
