...
- **Primary Configuration**: The project uses `models.json` as the single source of truth for model configurations.
  - This file defines available providers, models, API endpoints (`base_url`), and model-specific parameters like `temperature`, `max_tokens`, and `thinking` mode status.
  - The `active_model` field in `models.json` determines the default model used by the CLI.
- **Secrets Management**:
  - Sensitive API keys should be stored in `.env` (e.g., `ANTHROPIC_API_KEY`, `DEEPSEEK_API_KEY`).
  - `models.json` references these keys using `${VAR_NAME}` syntax (e.g., `"api_key": "${ANTHROPIC_API_KEY}"`).
  - Do not commit `models.json` if it contains hardcoded secrets. Use `models_template.json` for sharing configuration structures.
- **Model Switching**: Use the CLI command `/model <name>` to switch models dynamically. This updates `models.json` automatically.
